module ApplicationController::Performance
  ...
  def perf_gen_data_before_wait
    ...
    when "hourly", "daily"
      ...
      if @perf_record.kind_of?(MiqCimInstance)
        ...
        rpt.where_clause =  ["resource_type = ? and resource_id = ? and timestamp >= ? and timestamp <= ? and capture_interval_name = ?",
                             ...
                             to_dt,
                             interval_type]
      elsif %w(MiddlewareServer MiddlewareDatasource MiddlewareMessaging MiddlewareCamelContext).include?(@perf_record.class.name.demodulize)
    ...
    when "realtime"
      ...
      suffix = if %w(MiddlewareServer MiddlewareDatasource MiddlewareMessaging MiddlewareCamelContext)
